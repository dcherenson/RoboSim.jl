...
@kwdef struct BackupPlannerProblem{F, VW} <: RRTStar.AbstractProblem{SVector{3,F}}
...
function initialize_backup_planner(domain::Tuple{TV, TV}, landmarks::Vector{TV}, sample_space::MVector{3,F}, zones::Vector{W}, max_iter) where {F, TV, W<: World.AbstractUnsafeZone}
...
function update_backup_planner!(planner::BackupPlanner, new_fov::TS, max_iter) where {TS}
