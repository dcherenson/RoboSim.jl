...
function load_state_trajectory(file_path::String)
...
function load_ergodic_demo1_traj()

    # Load the saved file
    data = load_state_trajectory("../traj_data/Demo1_quad_full_state_trajectory.bin")

    # Extract time vector and state trajectory
    ts_loaded = data["time"]
    state_traj_loaded = data["state_traj"]

    # === Split loaded states into x, y, vx, vy ===
    x_vals = [s[1] for s in state_traj_loaded]
    y_vals = [s[2] for s in state_traj_loaded]
    vx_vals = [s[3] for s in state_traj_loaded]
    vy_vals = [s[4] for s in state_traj_loaded]

    # === Create cubic splines ===
    Δt = ts_loaded[2] - ts_loaded[1]  # Step size
    itp_x = cubic_spline_interpolation(0.0:Δt:Δt*(length(x_vals)-1), x_vals)
    itp_y = cubic_spline_interpolation(0.0:Δt:Δt*(length(y_vals)-1), y_vals)
    itp_vx = cubic_spline_interpolation(0.0:Δt:Δt*(length(vx_vals)-1), vx_vals)
    itp_vy = cubic_spline_interpolation(0.0:Δt:Δt*(length(vy_vals)-1), vy_vals)

    println("==== Trajectories loaded ==== ")

    return itp_x, itp_y, itp_vx, itp_vy, ts_loaded, state_traj_loaded

end



# === Function to get interpolated state at arbitrary time ===
# === Function to get interpolated state at arbitrary time ===

# 
function get_ergodic_state_at(t_query)
    if t_query > ts_loaded[end]  # if query time exceeds the maximum time
        return state_traj_loaded[end]  # just return the final state
    else
        x = itp_x(t_query)
        y = itp_y(t_query)
        vx = itp_vx(t_query)
...
function load_ergodic_demo2_traj()

    # Load the saved file
    println("About to load the trajectories")
    data = load_state_trajectory("../traj_data/Demo2_quad_full_state_trajectory.bin")
    ...
    # === Split loaded states into x, y, vx, vy ===
    r1_x_vals = [s[1] for s in state_traj_loaded[1]]
    r1_y_vals = [s[2] for s in state_traj_loaded[1]]
    r1_vx_vals = [s[3] for s in state_traj_loaded[1]]
    r1_vy_vals = [s[4] for s in state_traj_loaded[1]]

    # === Create cubic splines ===
    Δt = ts_loaded[2] - ts_loaded[1]  # Step size
    r1_itp_x = cubic_spline_interpolation(0.0:Δt:Δt*(length(r1_x_vals)-1), r1_x_vals)
    r1_itp_y = cubic_spline_interpolation(0.0:Δt:Δt*(length(r1_y_vals)-1), r1_y_vals)
    r1_itp_vx = cubic_spline_interpolation(0.0:Δt:Δt*(length(r1_vx_vals)-1), r1_vx_vals)
    r1_itp_vy = cubic_spline_interpolation(0.0:Δt:Δt*(length(r1_vy_vals)-1), r1_vy_vals);

    # === Split loaded states into x, y, vx, vy ===
    r2_x_vals = [s[1] for s in state_traj_loaded[2]]
    r2_y_vals = [s[2] for s in state_traj_loaded[2]]
    r2_vx_vals = [s[3] for s in state_traj_loaded[2]]
    r2_vy_vals = [s[4] for s in state_traj_loaded[2]]

    # === Create cubic splines ===
    Δt = ts_loaded[2] - ts_loaded[1]  # Step size
    r2_itp_x = cubic_spline_interpolation(0.0:Δt:Δt*(length(r2_x_vals)-1), r2_x_vals)
    r2_itp_y = cubic_spline_interpolation(0.0:Δt:Δt*(length(r2_y_vals)-1), r2_y_vals)
    r2_itp_vx = cubic_spline_interpolation(0.0:Δt:Δt*(length(r2_vx_vals)-1), r2_vx_vals)
    r2_itp_vy = cubic_spline_interpolation(0.0:Δt:Δt*(length(r2_vy_vals)-1), r2_vy_vals)

    println("==== Trajectories loaded for Demo 2 ==== ")
    return r1_itp_x, r1_itp_y, r1_itp_vx, r1_itp_vy, r2_itp_x, r2_itp_y, r2_itp_vx, r2_itp_vy, ts_loaded, state_traj_loaded

end 



function demo2_get_ergodic_state_at(t_query, quad_num)
    if t_query > ts_loaded[end]  # if query time exceeds the maximum time
        return state_traj_loaded[quad_num][end]  # just return the final state
    elseif quad_num == 1
        x = r1_itp_x(t_query)
        y = r1_itp_y(t_query)
        vx = r1_itp_vx(t_query)
        vy = r1_itp_vy(t_query)
        return (x, y, vx, vy)
    elseif quad_num == 2
        x = r2_itp_x(t_query)
        y = r2_itp_y(t_query)
        vx = r2_itp_vx(t_query)
        vy = r2_itp_vy(t_query)
        return (x, y, vx, vy)        
    end
end
...
# Run demo 2
itp_x, itp_y, itp_vx, itp_vy, ts_loaded, state_traj_loaded = load_ergodic_demo1_traj()
# r1_itp_x, r1_itp_y, r1_itp_vx, r1_itp_vy, r2_itp_x, r2_itp_y, r2_itp_vx, r2_itp_vy, ts_loaded, state_traj_loaded = load_ergodic_demo2_traj()
mission_type = "ERGO_DEMO1" # ERGO_DEMO2, ERGO_DEMO3
...
function DI_3D_get_desired_ergodic_trajectory(t0, traj_type, N, dt, param_type)

    if mission_type == "ERGO_DEMO1"
        DIXref = [zeros(6) for i=1:N]
        for i = 1:(N)
            ti = t0 + (i-1)*dt
            ergodic_des_state = get_ergodic_state_at(ti)
            DIXref[i][1:6] = [ergodic_des_state[1]; ergodic_des_state[2]; 7.0; ergodic_des_state[3]; ergodic_des_state[4]; 0.0]
        end

        # Control Input Trajectory
        DIUref = [[0; 0; 0.0] for i = 1:(N-1)]        
        return DIXref, DIUref
    elseif mission_type == "ERGO_DEMO2"
        DIXref = [zeros(6) for i=1:N]
        for i = 1:(N)
            ti = t0 + (i-1)*dt
            ergodic_des_state = demo2_get_ergodic_state_at(ti, param_type)
            DIXref[i][1:6] = [ergodic_des_state[1]; ergodic_des_state[2]; 7.0; ergodic_des_state[3]; ergodic_des_state[4]; 0.0]
        end
            # Control Input Trajectory
            DIUref = [[0; 0; 0.0] for i = 1:(N-1)]        
