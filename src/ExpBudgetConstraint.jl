module BudgetConstraintLostComm
...
mutable struct QuadObj{F}
    ...
    reroot::BackupPlanner{BackupPlannerProblem, RRTStar.Node{SVector{3,F}}} # reroot planner
    T_lost_comms::F                           # time when quad lost comms  
...
QuadObj() = QuadObj(
    ...
    BackupPlanner{BackupPlannerProblem, RRTStar.Node{SVector{3,Float64}}}(),   # reroot planner
    Float64,   # lost comms time
    String,    # flight mode
...
function get_candidate_traj!(integrator, t_k, state_k, traj_nom_k)
    ...
    velocity_heading = atan(state_kN_end[5], state_kN_end[4])
    dubins_state = SVector(state_kN_end[1], state_kN_end[2], velocity_heading)
    comp_time_b2b = @elapsed traj_b2b_kn = query_backup_planner(integrator.p[1].reroot, dubins_state)
    # println("here2")
...
function recompute_committed_timed_quads!(integrator)
    ...
    while t_S >= 0.0
        ...
        get_candidate_traj!(integrator, integrator.t, integrator.u, integrator.p[1].nom_traj)
...
function simulate(quad_init_state, t0, t_max, quad_id, reroot_planner, t_lost_comms)
    tspan = (t0, t_max)
